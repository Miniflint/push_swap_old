...
/*   Updated: 2022/07/15 08:47:56 by tgoel            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	__init__stack(t_all *all)
{
	...
	if (!all->stack_a->nb)
		handle_error("Couldn't malloc nb a", all);
	if (!all->stack_b)
		handle_error("Couldn't malloc nb b", all);
...
void	__init__all(t_all *all, int size)
{
	...
	if (!all->stack_a || !all->stack_b)
		handle_error("Failed to malloc stack a", all);
	if (!all->stack_b)
		handle_error("Failed to malloc stack b", all);
	all->max_size = size;
	all->a_sorted = 0;
	all->to_sort = all->max_size - all->a_sorted;
	all->formula_sort = (all->max_size / 6 + 22);
	__init__stack(all);
}
